                ...
                for p in soup.findAll('p'):
                    ...
                    if(p.getText() == 'There are over 1,200 national days. Don’t miss a single one. Celebrate Every Day with National Day Calendar!'):
                        ...
                        try:
                            ...
                            articleLink.append("".join(a.findAll(text=True)).replace("'", "").replace("\"", "").replace("\xa0", " ").replace("\u2122", " tm").replace("\xfb", "u").replace("\xf6", "o").replace("(adsbygoogle = window.adsbygoogle || []).push({});", ""))
